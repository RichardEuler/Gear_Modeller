...
classdef ansysIntegrationUtils < handle
    ...
    methods (Access = private)
        ...
        function journalPath = writeWorkbenchJournal(~, workDir, p)
            ...
            wl('');
            wl('# 2. Engineering Data — isotropic linear elastic "GearSteel"');
            wl('edSys = system1.GetContainer(ComponentName="Engineering Data")');
            wl('edSys.Edit()');
            wl('matl  = edSys.CreateMaterial(Name="GearSteel")');
            wl('prop  = matl.CreateProperty(Name="Elasticity", Behavior="Isotropic")');
            wl('prop.SetVariableProperty(VariableName="Young''s Modulus",');
            wl('                         Property="Unit", Value="MPa")');
            wl('prop.SetData(Index=-1, Variables=["Young''s Modulus"],');
            wl('             Values=[["%g [MPa]"]])', p.E_MPa);
            wl('prop.SetData(Index=-1, Variables=["Poisson''s Ratio"],');
            wl('             Values=[["%g"]])', p.nu);
            wl('edSys.Exit()');
            wl('');
            wl('# 3. Geometry via SpaceClaim IronPython');
            ...
            wl('geomSys.Exit()');
            wl('');
            wl('# 4. Mechanical setup + solve via IronPython');
